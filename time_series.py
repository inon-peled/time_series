...
class DeepLearnSpeeds:
    ...
    @staticmethod
    def __add_plot(model, dataset_predict_name, full_dataset, dataset_predict, offset_start, offset_end, plt_obj):
        predict = model.predict(dataset_predict)
        ...
        errors = full_dataset[offset_start:offset_end, :] - dataset_predict
        max_abs_err = max(enumerate(abs(errors)), key=lambda pair: pair[1])
        print(max_abs_err)
        rmse = sum(errors ** 2 / len(errors)) ** 0.5
        print('%s dataset: Maximum absolute error is %.2f ; Root of Mean Square Error is %.2f' %
              (dataset_predict_name, max_abs_err[1], rmse))
        return errors

    def __plot(self, model, dataset, train_x, test_x):
        # plot baseline and predictions
        fig1 = plt.figure()
        ax1 = fig1.add_subplot(111)
        ax1.plot(dataset)
        self.__add_plot(model, 'Train', dataset, train_x, self.__look_back, len(train_x) + self.__look_back, ax1)
        test_errors = self.__add_plot(model, 'Test', dataset, test_x, len(train_x) + (self.__look_back * 2) + 1,
                                      len(dataset) - 1, ax1)
...
if __name__ == '__main__':
    ...
    deep_learn_speeds.main('speeds1_h20000_t5000.csv')
